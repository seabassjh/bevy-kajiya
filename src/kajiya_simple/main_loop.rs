use bevy::window::HasRawWindowHandleWrapper;
use kajiya::{
...
impl SimpleMainLoopBuilder {
    ...
    pub fn build(self, window_builder: WindowBuilder, event_loop: EventLoop<()>, 
        raw_window_handle: HasRawWindowHandleWrapper
    ) -> anyhow::Result<SimpleMainLoop> {
        SimpleMainLoop::build(self, window_builder, event_loop,
            raw_window_handle
        )
    }
}
...
impl SimpleMainLoop {
    ...
    fn build(
        ...
        // kajiya::logging::set_up_logging(builder.default_log_level).expect("FUCK");
